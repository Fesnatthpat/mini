    ...
    if (empty($semester)) { // ตรวจสอบว่าผู้ใช้ได้กรอกข้อมูลภาคเรียนหรือไม่
        ...
        try {
            ...
            if ($existingSchedule) { // ตรวจสอบว่าพบข้อมูลตารางสอนในฐานข้อมูลหรือไม่
                ...
                $stmt->bindParam(":semester", $semester, PDO::PARAM_STR); // ผูกค่าภาคเรียนกับพารามิเตอร์ในคำสั่ง SQL
                $stmt->bindParam(":academic_year", $academic_year, PDO::PARAM_STR); // ผูกค่าปีการศึกษากับพารามิเตอร์ในคำสั่ง SQL
                $stmt->bindParam(":subject_id", $subject_id, PDO::PARAM_INT); // ผูกค่ารหัสวิชากับพารามิเตอร์ในคำสั่ง SQL
                $stmt->bindParam(":level", $level, PDO::PARAM_STR); // ผูกค่าระดับกับพารามิเตอร์ในคำสั่ง SQL
                $stmt->bindParam(":t_id", $t_id, PDO::PARAM_INT); // ผูกค่ารหัสอาจารย์กับพารามิเตอร์ในคำสั่ง SQL
                $stmt->bindParam(":room_id", $room_id, PDO::PARAM_INT); // ผูกค่าหมายเลขห้องกับพารามิเตอร์ในคำสั่ง SQL
                $stmt->bindParam(":teacher_date", $teacher_date); // ผูกค่าวันสอนกับพารามิเตอร์ในคำสั่ง SQL
                $stmt->bindParam(":teacher_time", $teacher_time, PDO::PARAM_STR); // ผูกค่าเวลาเรียนกับพารามิเตอร์ในคำสั่ง SQL
                ...
                header("location: Tutorial-Schedule.php"); // เปลี่ยนเส้นทางกลับไปยังหน้า add-subject.php
